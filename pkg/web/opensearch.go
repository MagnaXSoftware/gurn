package web

import (
	"net/http"
	"text/template"
)

const (
	// DEFAULT_IMAGE_URI string = "data:image/x-icon;base64,"
	NS_MOZ        string = "http://www.mozilla.org/2006/browser/search/"
	NS_OPENSEARCH string = "http://a9.com/-/spec/opensearch/1.1/"
)

var openSearchContentType = []string{"application/opensearchdescription+xml; charset=utf-8"}
var openSearchDescriptionTmpl = template.Must(template.New("opensearchdescription.xml").Parse(`<OpenSearchDescription xmlns="http://a9.com/-/spec/opensearch/1.1/"
                       xmlns:moz="http://www.mozilla.org/2006/browser/search/">
  <ShortName>{{ .ShortCode }}</ShortName>
  <Description>{{ .Description }}</Description>
  <InputEncoding>UTF-8</InputEncoding>
  <OutputEncoding>UTF-8</OutputEncoding>
  {{ range .Images }}<Image width="{{ .Width }}" height="{{ .Height }}">{{ .URI }}</Image>{{ end }}
  <Url type="text/html" template="{{ .SearchUrl }}"/>
  {{ if .Self }}<Url type="application/opensearchdescription+xml" rel="self" template="{{ .Self }}" />{{ end }}
  <moz:SearchForm>{{ .SearchUrl }}</moz:SearchForm>
</OpenSearchDescription>
`))

type OpenSearchDescription struct {
	ShortCode   string
	Description string
	Images      []OpenSearchImage
	Self        string
	SearchUrl   string
}

func (r *OpenSearchDescription) Render(w http.ResponseWriter) error {
	r.WriteContentType(w)
	return openSearchDescriptionTmpl.Execute(w, r)
}

func (r *OpenSearchDescription) WriteContentType(w http.ResponseWriter) {
	header := w.Header()
	if val := header["Content-Type"]; len(val) == 0 {
		header["Content-Type"] = openSearchContentType
	}
}

type OpenSearchImage struct {
	Height int    `xml:"height,attr"`
	Width  int    `xml:"width,attr"`
	URI    string `xml:",chardata"`
}
